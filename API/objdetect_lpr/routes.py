...
classesName = "lpr"
objdetect_lpr = Blueprint(f'objdetect_{classesName}', __name__, url_prefix=f'/od/{classesName}')

@objdetect_lpr.errorhandler(Exception)
def error_500(error):
    response = dict(message="500 Error, {0}".format(error), detail=traceback.format_exc())
    return jsonify(response), 500

@objdetect_lpr.errorhandler(KeyError)
def error_400(error):
...
@objdetect_lpr.route('/image_detect/<orc>', methods=['POST'], defaults={'orc': None})
@objdetect_lpr.route('/image_detect', methods=['POST'])
def image_detect(ocr):
    ...
    if imageFileName != "":
        ...
        detectedImage = utils.draw_bbox_by_classes(original_image, pred_bbox, classesName=classesName, read_plate=ocr)
...
@objdetect_lpr.route('/video_upload', methods=['POST'])
def video_upload():
...
def gen_video_stream(filepath, filename, ocr):
    ...
    while(vid.isOpened()):
        ...
        pred_bbox = [boxes.numpy(), scores.numpy(), classes.numpy(), valid_detections.numpy()]
        detectedImage = utils.draw_bbox_by_classes(frame, pred_bbox, classesName=classesName, read_plate=ocr)
        curr_time = time.time()
...
@objdetect_lpr.route('/video_detect/<filename>/<ocr>', methods=['GET'], defaults={'orc': None})
@objdetect_lpr.route('/video_detect/<filename>', methods=['GET'])
def video_detect(filename, ocr):
    
    try:
      filepath = os.path.join(app.config["VIDEO_UPLOADS"], filename)
      if os.path.isfile(filepath):
        return Response(gen_video_stream(filepath, filename, ocr), mimetype='multipart/x-mixed-replace; boundary=frame')
      else:
        return jsonify({"response": "FileNotFoundError"}), 400
    except Exception as e:
