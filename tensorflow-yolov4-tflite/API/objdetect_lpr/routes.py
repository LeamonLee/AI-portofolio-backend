...
@objdetect_lpr.route('/image_detect/ocr', methods=['POST'])
@objdetect_lpr.route('/image_detect', methods=['POST'])
def image_detect_route():

    isOCR = False
    if 'ocr' in request.url_rule.rule:
        isOCR = True
        print("isOCR")
    ...
    if imageFileName != "":
        ...
        return image_detect(saveImagePath, detectedImagePath, modelInfer, classesName, isOCR, **dctODKwargs)
...
# def gen_video_stream(filepath, filename, isOCR):
...
@objdetect_lpr.route('/video_detect/<filename>/ocr', methods=['GET'])
...
def video_detect(filename):
    ...
    try:
        ...
        if os.path.isfile(filepath):
            ...
            return Response(gen_video_stream(filepath, detectedVideoPath, classesName, isOCR, **dctODKwargs), mimetype='multipart/x-mixed-replace; boundary=frame')
        else:
            return jsonify({"response": "FileNotFoundError"}), 400
    except Exception as e:
