...
classesName = "coronavirus"
objdetect_coronavirus = Blueprint(f'objdetect_{classesName}', __name__, url_prefix=f'/od/{classesName}')

@objdetect_coronavirus.errorhandler(Exception)
def error_500(error):
    response = dict(message="500 Error, {0}".format(error), detail=traceback.format_exc())
    return jsonify(response), 500

@objdetect_coronavirus.errorhandler(KeyError)
def error_400(error):
...
@objdetect_coronavirus.route('/image_detect', methods=['POST'])
...
@objdetect_coronavirus.route('/video_upload', methods=['POST'])
def video_upload():
...
@objdetect_coronavirus.route('/video_detect/<filename>', methods=['GET'])
