...
def recognize_plate(img, coords):
    ...
    ret, thresh_test = cv2.threshold(blur, 0, 255, cv2.THRESH_OTSU | cv2.THRESH_BINARY_INV)
    cv2.imshow("Otsu Threshold Test", thresh_test)
    cv2.waitKey(0)
...
def draw_bbox_by_classes(image, bboxes, classesName: str, show_label=True, read_plate=False):
    try:
        classesFilepath = cfg.YOLO.MYCLASSES[classesName]["namesPath"]
        classNames = read_class_names(classesFilepath)
        return draw_bbox(image, bboxes, classNames, show_label, read_plate)
    except KeyError as e:
        print("KeyError: ", e)
        return None


def draw_bbox(image, bboxes, classes=read_class_names(cfg.YOLO.CLASSES), show_label=True, read_plate=False):
